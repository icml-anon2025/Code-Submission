...
def mean_imputation(X_train, X_test, y_train, y_test, seed):
    ...
    f1_MI = f1_score(y_test, y_pred, average='binary', pos_label=2)
    end_time = time.time()
...
def knn_imputation(X_train, X_test, y_train, y_test, seed):
    ...
    f1_KNN = f1_score(y_test, y_pred,average='binary', pos_label=2)
    end_time = time.time()
...
def original_zero_imputation(OG_X_train, X_test, OG_y_train, y_test, seed):
    ...
    f1_OG = f1_score(y_test, y_pred,average='binary', pos_label=2)
    end_time = time.time()
    training_time_OG = end_time - start_time
    return accuracy_OG, f1_OG, training_time_OG

if __name__ == '__main__':
    df = pd.read_csv("./Final-Datasets/BreastCancer.csv")
    label = '10'
    X, y = get_Xy(df, label)
    ...
    f1_MM = f1_score(y_test, y_pred_MM,average='binary', pos_label=2)
    ...
    f1_MM_KNN = f1_score(y_test, model.predict(X_test),average='binary', pos_label=2)
    end_time = time.time()
    training_time_MM_KNN = end_time - start_time


    filename = os.path.join('./SVM/Final-Results', f"Breast_Cancer_MM.txt")
