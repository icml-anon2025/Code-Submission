...
class Tokenizer(nn.Module):
    ...
    def forward(self, x_num: Tensor, x_cat: ty.Optional[Tensor]) -> Tensor:
        ...
        if x_cat is not None:
            x = x[:, np.newaxis, :, :]
            x = x.permute(0, 1, 3, 2)
            # print("x_cat min:", x_cat.min().item())
            # print("x_cat max:", x_cat.max().item())
            # print("category_offsets:", self.category_offsets)
            # print("Computed index max:", (x_cat + self.category_offsets[None]).max().item())
            # print("Embedding weight size:", self.category_embeddings.weight.shape[0])
            x = torch.cat(
    ...
    def recover(self, Batch, d_numerical):
        ...
        actual_size = Batch.numel()  # Actual number of elements
...
class CSDI_base(nn.Module):
    def __init__(self, exe_name, target_dim, config, device):
        ...
        if exe_name == "online_ed":
            with open("./data_online_ed_ft_MI/transformed_columns.pk", "rb") as f:
                cont_list, num_cate_list = pickle.load(f)
        if exe_name == "crime":
            with open("./data_crime_ft_MI/transformed_columns.pk", "rb") as f:
                cont_list, num_cate_list = pickle.load(f)
